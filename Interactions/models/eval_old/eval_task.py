...
class EvalTask(Eval):
    ...
    def evaluate(cls, env, model, r_idx, resnet, traj_data, args, lock, successes, failures, results):
        ...
        man_actions = ['PickupObject', 'SliceObject', 'OpenObject', 'PutObject', 'CloseObject', 'ToggleObjectOn', 'ToggleObjectOff', '<<stop>>', '<<pad>>']
        ...
        while not done:
            ...
            # print(action, lang_index)
            # action = man_actions[0]
            ...
            if action == cls.MANIPULATE_TOKEN and prev_action == cls.MANIPULATE_TOKEN:
                ...
                if rotate_count == 5:
                    ...
                    rotate_count=0                    
                action = model.vocab['action_low'].index2word(torch.argmax(dist_action*action_mask))
                print(action, "action after man loop break")
                if 'Look' in action:
                    look_count+=1
                if 'Rotate' in action:
                    rotate_count+=1
