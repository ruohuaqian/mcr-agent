...
class Module(Base):
    ...
    def featurize(self, batch, load_mask=True, load_frames=True):
        ...
        for ex, swapColor in batch:
            ...
            #['CoolObject', 'HeatObject', 'PutObject', 'PickupObject', CleanObject', 'ToggleObject']
            subgoal_analysis = 'ToggleObject'

            alow_list = [a['action'] for a in ex['num']['action_low']]
            cool_action_high = (action_high == self.vocab['action_high'].word2index(subgoal_analysis, train=False)).astype(np.int64)
            print('action_high', action_high)
            # print("cool_ac_first_high", low_to_high_idx[cool_action_high.nonzero()[0][0]])
            cool_actions = np.array(alow_list)[cool_action_high.nonzero()[0].astype(int)]

            # cool_ac_first_high = action_high.index()

            print("cool_action_high", cool_action_high)
            print("alow_list", alow_list)
            print("cool_actions int", cool_actions)
            ...
            if load_mask:
                ...
                for a in ex['plan']['low_actions']:
                    ...
                    if a['high_idx'] == (high_idx+1):
                        ...
                        obj_list.append(self.vocab['objnav'].word2index((label[4].split('_')[0] if len(label) >= 5 else label[0]).lower(), train=False))
                        high_idx+=1 

                new_obj_list = [obj_list[o+1] for o, obj in enumerate(obj_list) if (obj == self.vocab['objnav'].word2index('<<nav>>'))]
               
                feat['objnav'].append(new_obj_list)
                feat['action_low_mask_label'].append(indices)
                

            if len(cool_actions) > 0:
                print('obj_high_indices', obj_high_indices)
                print('action_high_order', action_high_order)
                print('indices', indices)
                print((np.array(obj_high_indices) == low_to_high_idx[cool_action_high.nonzero()[0][0]]).astype(int).nonzero()[0])
                cool_objs = np.array(indices)[(np.array(obj_high_indices) == low_to_high_idx[cool_action_high.nonzero()[0][0]]).astype(int).nonzero()[0]]
                print('cool_objs', cool_objs)
                print('cool_objs', [classes[c] for c in cool_objs])
                print("cool_actions", self.vocab['action_low'].index2word(list(cool_actions)))

                string = [a+','+c for a,c in zip(self.vocab['action_low'].index2word(list(cool_actions)), [classes[c] for c in cool_objs])]
                print(string)

                print(indices[list(action_high_order).index(self.vocab['action_high'].word2index(subgoal_analysis))])
                with open(subgoal_analysis+'.csv', 'a') as f:
                    f.write('\n' + str(string))
            ...
            if load_frames and not self.test_mode:
                ...
                if self.orientation:
                    ...
                    feat['frames_right'][-1] = torch.cat([
                        feat['frames_right'][-1], get_orientation('right').repeat(len(feat['frames_right'][-1]), 1,1,1)
                    ], dim=1)
