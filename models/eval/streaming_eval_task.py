...
class StreamingEvalTask(Eval):
        ...
        if obj_name[0] in objects_present:
            ...
            if scr > 0.3:
                ...
                while not done:
                    ...
                    m_pred = model.extract_preds(m_out, [(data['ex'], False)], feat1, clean_special_tokens=False)
    ...
    def evaluate_streaming(cls, env, model, r_idx, resnet, data, args, lock, successes, failures, results, task):
        ...
        os.makedirs(os.path.join(save_dir, task['task'], str(task['repeat_idx'])), exist_ok=True)
