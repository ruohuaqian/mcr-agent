...
class ConvFrameMaskDecoderProgressMonitor(nn.Module):
    ...
    def __init__(self, emb, dframe, dhid, pframe=300,
        ...
        self.mask_dec = nn.Sequential(
            nn.Linear(dhid+dhid+dframe+demb, dhid), nn.ReLU(),
            nn.Linear(dhid, 119)
        )
    ...
    def step(self, enc_instr, frame, e_t, state_tm1_instr):
        vis_feat_t = frame
        ...
        return action_t, mask_t, state_t_instr, lang_attn_t_instr

    def forward(self, enc_instr, frames, gold=None, max_decode=150, state_0_instr=None):
        ...
        for t in range(max_t):
            action_t, mask_t, state_t_instr, attn_score_t_instr = self.step(enc_instr, frames[:, t], e_t, state_t_instr)
            masks.append(mask_t)
...
class ConvFrameMaskDecoderProgressMonitorPanoramicConcat(nn.Module):
    ...
    def __init__(self, emb, dframe, dhid, pframe=300,
        ...
        self.subgoal = nn.Linear(dhid+dhid+dframe*5+demb, 1)
        self.progress = nn.Linear(dhid+dhid+dframe*5+demb, 1)
    ...
    def forward(self, enc_goal, enc_instr, frames, frames_left, frames_up, frames_down, frames_right, gold=None, max_decode=150, state_0_goal=None, state_0_instr=None):
        ...
        progresses = []
        for t in range(max_t):
            action_t, mask_t, state_t_goal, state_t_instr, attn_score_t_goal, attn_score_t_instr, subgoal_t, progress_t = self.step(enc_goal, enc_instr, frames[:, t], frames_left[:, t], frames_up[:, t], frames_down[:, t], frames_right[:, t], e_t, state_t_goal, state_t_instr)
            masks.append(mask_t)
            actions.append(action_t)
