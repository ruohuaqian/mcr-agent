...
import torch
import numpy as np
import models.nn.vnn_sub as vnn
import collections
...
class Module(Base):
    ...
    def featurize(self, batch, action_high_order, load_mask=True, load_frames=False):
        '''
        ...
        for ex, swapColor in batch:
            ...
            if not self.test_mode:
                sub_action_high = (action_high == self.vocab['action_high'].word2index(subgoal_analysis, train=False)).astype(np.int64)
            ...
            feat['lang_instr_sep'] = lang_instr_sep
            ...
            if not self.test_mode:
                ...
                for sii, s in enumerate(sah):
                    ...
                    if not self.test_mode:
                        ...
                        so = np.array(indices)[(np.array(obj_high_indices) == np.array(low_to_high_idx)[s][0]).astype(int).nonzero()[0]]
        ...
        for k, v in feat.items():
            if k in {'lang_goal', 'sub_lang'}:
                ...
                feat[k] = {'seq_list':v, 'seqs':seqs, 'emb':embed_seq, 'pi': packed_input, 'seq_len':seq_lengths}

            elif k in {'lang_instr_sep'}:
                # language embedding and padding
                seqs = [torch.tensor(vv, device=device) for vv in v]
                embed_seq = [self.emb_word(pad_seq) for pad_seq in seqs]
                feat[k] = {'seq_list':v, 'seqs':seqs, 'emb':embed_seq}
    ...
    def serialize_lang_action(self, feat, action_high_order):
        ...
        import copy
        feat['num']['lang_instr_sep'] = copy.deepcopy(feat['num']['lang_instr'])
    ...
    def step(self, feat, eval_idx, prev_action=None):
        ...
        if self.r_state['cont_lang_instr'] is None and self.r_state['enc_lang_instr'] is None:
            ...
            packed_input = feat['lang_instr_sep']['emb'][eval_idx]

            self.r_state['cont_lang_instr'], self.r_state['enc_lang_instr'] = self.encode_lang_instr(packed_input.unsqueeze(0))
