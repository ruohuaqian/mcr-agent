...
class EvalTask(Eval):
    ...
    def run(cls, model, resnet, task_queue, args, lock, successes, failures, results):
        ...
        while True:
            ...
            try:
                ...
                loss, correct, num = cls.evaluate(env, model, r_idx, resnet, traj, args, lock, successes, failures, results)
        ...
        print('mean_loss', total_loss/total_num)
        # stop THOR
        model_name = args.model_path.split('/')[-1]
        with open(f'{args.eval_split}_results.txt', 'a') as f:

            string = f'\n{model_name}\t\ttotal_num={total_num}\ttotal_correct={total_correct}\ttotal_loss={total_loss}\taccuracy={total_correct/total_num}\tmean_loss={total_loss/total_num}'
            f.write(string)
        # env.stop()
    ...
    def evaluate(cls, env, model, r_idx, resnet, traj_data, args, lock, successes, failures, results):
        ...
        # out = out['out_sub']
        subgoal_mask = torch.ones(len(model.vocab['action_high']), dtype=torch.float).cuda()
        subgoal_mask[model.vocab['action_high'].word2index(['<<pad>>', '<<seg>>', '<<stop>>', 'diningtable', 'knife', 'lettuce', 'fridge', 
        ...
        # preds = F.softmax(out)
        preds = torch.argmax(subgoal_mask*out['out_sub'], dim=1)

        num = len(preds)

        correct = (preds == feat['action_high_order'].cuda()).sum()
        
        loss = model.compute_loss(out, [(traj_data, False)], feat)['action_high_order']
